...
class AdaLNTransformer(nn.Module):
    def __init__(self, 
        ...
        self.layers = nn.ModuleList([AdaLNSelfAttn(
            ...
            num_heads=nhead,
            shared_aln=shared_aln) for i in range(nlayer)])
...
class SkipAdaLNTransformer(nn.Module):
    def __init__(self, d_model:int, nhead:int, nlayer:int, d_ffn:int, d_cond:int, dropout:float=0.1, shared_aln: bool = False):
        super().__init__()
        self.d_model = d_model
        self.d_cond = d_cond
        self.nhead = nhead
        ...
        self.in_layers = nn.ModuleList([AdaLNSelfAttn(
            ...
            norm_layer=self.norm_layer,
            num_heads=nhead,
            shared_aln=shared_aln) for i in range(self.nlayer_in)])
